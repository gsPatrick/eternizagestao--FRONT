// Configuração white label por tenant (prefeitura/concessionária).
// Em produção, isto vem da API resolvida pelo subdomínio (tenant.cores, logo).
// O acento troca por cliente; as cores semânticas (sucesso/erro) NUNCA trocam.

export const TENANTS = [
  {
    id: "eterniza",
    name: "Eterniza Gestão",
    brandLead: "Eterniza",
    brandTail: "Gestão",
    subdomain: "demo.eterniza.com.br",
    accent: "#032e59",
    accentRgb: "3, 46, 89",
    accentBright: "#0a4a8c",
    accentDeep: "#02223f",
  },
  {
    id: "sao-paulo",
    name: "Prefeitura de São Paulo",
    brandLead: "São Paulo",
    brandTail: "Cemitérios",
    subdomain: "saopaulo.eterniza.com.br",
    accent: "#7a1b1b",
    accentRgb: "122, 27, 27",
    accentBright: "#a83232",
    accentDeep: "#571010",
  },
  {
    id: "guarulhos",
    name: "Prefeitura de Guarulhos",
    brandLead: "Guarulhos",
    brandTail: "Memorial",
    subdomain: "guarulhos.eterniza.com.br",
    accent: "#1a5c3a",
    accentRgb: "26, 92, 58",
    accentBright: "#288a58",
    accentDeep: "#123f28",
  },
  {
    id: "osasco",
    name: "Prefeitura de Osasco",
    brandLead: "Osasco",
    brandTail: "Cemitérios",
    subdomain: "osasco.eterniza.com.br",
    accent: "#5b3a8c",
    accentRgb: "91, 58, 140",
    accentBright: "#7a52b8",
    accentDeep: "#412963",
  },
  {
    id: "campinas",
    name: "Prefeitura de Campinas",
    brandLead: "Campinas",
    brandTail: "Memorial",
    subdomain: "campinas.eterniza.com.br",
    accent: "#0f6b6b",
    accentRgb: "15, 107, 107",
    accentBright: "#189494",
    accentDeep: "#0a4a4a",
  },
  {
    id: "santos",
    name: "Prefeitura de Santos",
    brandLead: "Santos",
    brandTail: "Cemitérios",
    subdomain: "santos.eterniza.com.br",
    accent: "#12507a",
    accentRgb: "18, 80, 122",
    accentBright: "#1d70a8",
    accentDeep: "#0c3854",
  },
  {
    id: "ribeirao",
    name: "Prefeitura de Ribeirão Preto",
    brandLead: "Ribeirão",
    brandTail: "Memorial",
    subdomain: "ribeirao.eterniza.com.br",
    accent: "#a8532b",
    accentRgb: "168, 83, 43",
    accentBright: "#c96e42",
    accentDeep: "#7c3c1f",
  },
  {
    id: "sorocaba",
    name: "Prefeitura de Sorocaba",
    brandLead: "Sorocaba",
    brandTail: "Cemitérios",
    subdomain: "sorocaba.eterniza.com.br",
    accent: "#3a3f8c",
    accentRgb: "58, 63, 140",
    accentBright: "#5257b8",
    accentDeep: "#292d63",
  },
];

export const DEFAULT_TENANT = TENANTS[0];

export function getTenant(id) {
  return TENANTS.find((t) => t.id === id) || DEFAULT_TENANT;
}

/* ------------------------------------------------------------------------- *
 * Normalização da API pública → shape do FRONT.
 *
 * A fonte da verdade das cidades é `GET /public/tenants`
 * (→ { id, name, subdomain, primaryColor, secondaryColor, logoUrl }).
 * Aqui convertemos esse shape para o shape que os componentes já consomem
 * (accent/accentRgb/accentBright/accentDeep + brandLead/brandTail). O FRONT
 * continua sendo o dono do formato; a API só alimenta os valores.
 * ------------------------------------------------------------------------- */

function hexToRgb(hex) {
  const h = String(hex || "").replace("#", "").trim();
  const full = h.length === 3 ? h.split("").map((c) => c + c).join("") : h;
  const n = parseInt(full || "000000", 16);
  return [(n >> 16) & 255, (n >> 8) & 255, n & 255];
}

function channel(n) {
  return Math.max(0, Math.min(255, Math.round(n))).toString(16).padStart(2, "0");
}

// Escurece um acento por um fator (mantém a proporção da cor navy → deep).
function darkenRgb([r, g, b], factor) {
  return `#${channel(r * factor)}${channel(g * factor)}${channel(b * factor)}`;
}

// "Prefeitura de São Paulo" → { brandLead: "São Paulo", brandTail: "Cemitérios" }
function deriveBrand(name) {
  const raw = String(name || "").trim();
  const city = raw.replace(/^Prefeitura\s+(?:de\s+|d[oa]s?\s+)?/i, "").trim() || raw;
  return { brandLead: city, brandTail: "Cemitérios" };
}

/**
 * Converte um tenant público da API no objeto que o FRONT usa.
 * `id` recebe o subdomínio (é o slug das URLs, ex.: "guarulhos") — o UUID
 * real vai em `tenantId`. `apiSubdomain` é o subdomínio "cru" para o header
 * X-Tenant-Subdomain; `subdomain` é o domínio de exibição.
 */
export function normalizeApiTenant(t) {
  const accent = t.primaryColor || DEFAULT_TENANT.accent;
  const rgb = hexToRgb(accent);
  const { brandLead, brandTail } = deriveBrand(t.name);
  return {
    id: t.subdomain,
    tenantId: t.id,
    name: t.name,
    brandLead,
    brandTail,
    subdomain: `${t.subdomain}.eterniza.com.br`,
    apiSubdomain: t.subdomain,
    accent,
    accentRgb: rgb.join(", "),
    accentBright: t.secondaryColor || accent,
    accentDeep: darkenRgb(rgb, 0.68),
    logoUrl: t.logoUrl || null,
  };
}

// Resolve o tenant de uma lista pelo slug (== subdomínio). Sem match → null.
export function resolveTenant(list, slug) {
  if (!slug) return null;
  return (list || []).find((t) => t.id === slug || t.apiSubdomain === slug) || null;
}
