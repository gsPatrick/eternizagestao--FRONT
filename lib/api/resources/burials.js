...
// Usa o helper compartilhado: aqui a origem vinha da env SEM fallback, então em
// produção sem a variável virava string vazia e o caminho resolvia contra o
// FRONT — que não serve esses arquivos. Mesmo defeito que escondeu a ortofoto.
export { fileHref as fileUrl } from "@/lib/api/file-url";
...
export function adaptBurialRow(b) {
  return {
    ...
    // geometria REAL do jazigo (lat/lng) para o mapa do detalhe (GraveMap)
    cemeteryId: b.grave?.cemeteryId || null,
    graveStatusSlug: b.grave?.status?.slug || null,
    geoPolygon: b.grave?.geoPolygon || null,
    latitude: b.grave?.latitude ?? null,
    longitude: b.grave?.longitude ?? null,
  };
}
