...
// Formatação de moeda compartilhada — usada pela tela de Cobranças e pela aba
// Financeiro da sepultura, para que os valores apareçam iguais nos dois lugares.
...
export function normalizeBilling(raw) {
  ...
  return {
    ...
    // total já calculado pela API (amount - desconto + multa + juros)
    total: num(raw.totalAmount),
    // vencimento cru (YYYY-MM-DD) — útil p/ ordenar sem reparsear o formato BR
    dueISO: raw.dueDate ? String(raw.dueDate).slice(0, 10) : null,
