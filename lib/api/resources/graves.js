...
/**
 * Gavetas EM LOTE ("1, 2, 3") de um mesmo bloco.
 * Um número já existente não aborta os demais — a resposta traz
 * { created, existing, failed } para a tela dizer o que entrou e o que já havia.
 */
export const createDrawers = (parentGraveId, numbers) =>
  api.post("/graves/drawers", { parentGraveId, numbers });
export const updateGrave = (id, body) => api.patch(`/graves/${id}`, body);
export const removeGrave = (id, { force = false } = {}) =>
  api.del(`/graves/${id}${force ? "?force=true" : ""}`);

/** O que a exclusão da sepultura arrasta junto (para a confirmação na tela). */
export const getGraveDeleteImpact = (id, opts) =>
  api.get(`/graves/${id}/delete-impact`, opts);

/**
 * Fotografia da SEPULTURA (campo "Fotografia" do formulário do cliente). Lê o
 * File como base64 e envia ao endpoint dedicado, que persiste no storage e
 * grava grave.photoUrl. Mesmo contrato da foto do sepultado.
 *   POST /graves/:id/photo { contentBase64, fileName, mimeType } → { photoUrl }
 */
export function uploadGravePhoto(id, file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      try {
        ...
        resolve(await api.post(`/graves/${id}/photo`, {
          contentBase64,
          fileName: file.name,
          mimeType: file.type,
        }));
...
// "Perpétuo" tem ACENTO: /perpet/ NÃO casa com "perp-é-tuo". Normaliza os
// diacríticos antes de testar — senão a sepultura perpétua era tratada como
// temporária (concessão errada e certidão nunca emitida).
export function isPerpetualUse(value) {
...
export function adaptGraveRow(g) {
  ...
  return {
    ...
    // registro completo: a edição pré-preenche o formulário sem ir buscar de novo
    raw: g,
  };
}
