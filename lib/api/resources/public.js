 ...
 *   GET /public/search                    → busca ampla (q) + filtros (nome,
 *                                            quadra, lote, jazigo, situacao,
 *                                            documento = nº da certidão de óbito)
 ...
 * A API (q) já detecta e casa em vários campos — nome do SEPULTADO, código do
 * jazigo, quadra, lote e situação — então basta encaminhar o texto como `q`.
 * LGPD: não há mais busca por proprietário/responsável nem por CPF/RG.
 */
 ...
 * Ignora campos vazios. Chaves: quadra, lote, jazigo, situacao, documento
 * (documento = apenas o nº da certidão de óbito; CPF/RG não são mais aceitos
 * como critério público — LGPD).
 */
export function buildFilterParams(filters = {}) {
  const out = {};
  for (const key of ["quadra", "lote", "jazigo", "situacao", "documento"]) {
...
// Agenda pública (velórios/sepultamentos/exumações do dia em diante).
// → [{ id, type, title, dateTime, place, deceasedName, cemeteryId, cemeteryName }]
// `cemeteryId` é OPCIONAL: sem ele a API agrega TODOS os cemitérios da cidade.
// Aceita janela opcional em `params` ({ from, to, limit }).
// no-store: a consulta pública tem de refletir a agenda ATUAL (novo sepultamento
// aparece na hora), sem servir uma resposta cacheada pelo navegador.
export const getPublicAgenda = (cemeteryId, { tenant, params, ...opts } = {}) =>
  api.get(cemeteryId ? `/public/cemeteries/${cemeteryId}/agenda` : "/public/agenda", {
    params,
    tenant,
    auth: false,
    cache: "no-store",
...
// no-store: resultado sempre fresco (um sepultado recém-cadastrado precisa
// aparecer na busca pública imediatamente).
export const searchPublic = (params, { tenant, ...opts } = {}) =>
  api.get("/public/search", { params, tenant, auth: false, meta: true, cache: "no-store", ...opts });

// Mapa do cemitério: ortofoto ativa + camadas de polígonos
// no-store: reflete a ortofoto/posição e as sepulturas ATUAIS no mapa da consulta.
export const getCemeteryMap = (cemeteryId, { tenant, ...opts } = {}) =>
  api.get(`/public/cemeteries/${cemeteryId}/map`, { tenant, auth: false, cache: "no-store", ...opts });
