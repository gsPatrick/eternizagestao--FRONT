...
// Configurável pela mesma env do resto do front para não divergir novamente.
...
// EDITA a cidade (super_admin). body com só os campos editáveis
// (name, primaryColor, secondaryColor, logoUrl, cnpj, phone, email, documentHeader).
// O subdomínio é IMUTÁVEL — não deve ser enviado.
export const updateTenant = (id, body) => api.patch(`/tenants/${id}`, body);
...
// (super_admin) DEFINE a senha do ADMINISTRADOR da cidade — digita a nova senha,
// sem depender de e-mail. Endpoint super_admin-only que NÃO usa header de tenant:
// o tenantId alvo vai no corpo (POST /users/tenant-admin-password). Sem `userId`,
// a API alveja o admin mais antigo da cidade (o administrador provisionado).
export const setTenantAdminPassword = (tenantId, password) =>
  api.post("/users/tenant-admin-password", { tenantId, password });

// Remoção (soft delete) — usada apenas para limpar cidades de teste.
export const removeTenant = (id) => api.del(`/tenants/${id}`);

/**
 * Faz upload da LOGO de uma cidade (super_admin). Lê o File como base64
 * (data URL → separa o prefixo `data:...;base64,`) e envia ao endpoint dedicado,
 * que persiste no storage e grava tenant.logoUrl.
 *   POST /tenants/:id/logo → { logoUrl }  (assinado, algo em /files/...)
 * @param {string} id    id da cidade (precisa já existir)
 * @param {File}   file  arquivo de imagem (PNG/JPEG/SVG)
 * @returns {Promise<{ logoUrl: string }>}
 */
export function uploadTenantLogo(id, file) {
    ...
    reader.onload = async () => {
      try {
        ...
        const contentBase64 = result.includes(",")
          ...
          : result;
        const data = await api.post(`/tenants/${id}/logo`, {
          contentBase64,
          fileName: file.name,
          mimeType: file.type,
        });
