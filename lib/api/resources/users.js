 ...
 *   PATCH  /users/:id                 edita (name, email, role, roleId)
 *   PATCH  /users/:id/password        DEFINE a senha direto (admin digita a nova)
 ...
 *
 * `role` (admin|operador|consulta) é o teto fixo; `roleId` (opcional) aponta um
 * PERFIL customizado que refina o acesso. Ao enviar roleId, a API grava o
 * baseRole do perfil em `role` — o authorize das rotas nunca muda.
...
export function adaptUser(u, currentUserId) {
  ...
  return {
    ...
    role: u.role, // teto fixo (baseRole) — sempre presente
    // Perfil customizado (opcional): nome/permissões efetivas quando vinculado.
