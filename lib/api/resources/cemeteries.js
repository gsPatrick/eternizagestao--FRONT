...
// no-store: o dropdown de quadra/lote (cadastro de sepultura) tem de mostrar a
// estrutura ATUAL — uma quadra/lote recém-criada aparece na hora.
export const getStructure = (cemeteryId, opts = {}) =>
  api.get(`/cemeteries/${cemeteryId}/structure`, { cache: "no-store", ...opts });
...
// atualização por nível — usada para salvar a geometria (geoPolygon) da camada
// desenhada no mapa (quadra/rua/lote → camada de navegação).
export const updateBlock = (id, body) => api.patch(`/blocks/${id}`, body);
export const updateStreet = (id, body) => api.patch(`/streets/${id}`, body);
export const updateLot = (id, body) => api.patch(`/lots/${id}`, body);

// salva a geometria de uma feature de estrutura pelo tipo (block|street|lot)
export const setStructureGeometry = (kind, id, geoPolygon) => {
  const fn = kind === "block" ? updateBlock : kind === "street" ? updateStreet : updateLot;
  return fn(id, { geoPolygon });
};

// achata a árvore da estrutura em uma lista de features demarcáveis no mapa
// → [{ kind, id, label, hasGeo }] (quadras, ruas e lotes)
export function flattenStructure(tree) {
  ...
  blocks.forEach((b) => {
    out.push({
      kind: "block",
      id: b.id,
      label: `Quadra ${b.name || b.code || ""}`.trim(),
      hasGeo: Boolean(b.geoPolygon),
    });
    (b.streets || []).forEach((s) => {
      out.push({
        kind: "street",
        id: s.id,
        label: `Rua ${s.name || s.code || ""}`.trim(),
        hasGeo: Boolean(s.geoPolygon),
      });
      (s.lots || []).forEach((l) => {
        out.push({
          kind: "lot",
          id: l.id,
          label: `Lote ${l.code || l.name || ""}`.trim(),
          hasGeo: Boolean(l.geoPolygon),
        });
      });
    });
  });
