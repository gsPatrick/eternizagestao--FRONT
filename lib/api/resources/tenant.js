...
export function toFormState(t = {}) {
  ...
  return {
    ...
    // *Url = ASSINADA (?token&exp), só para exibir no <img>
    logoUrl: t.logoUrl || "",
    // imagens da página pública da cidade (topo e rodapé)
    heroImageUrl: t.heroImageUrl || "",
    footerImageUrl: t.footerImageUrl || "",
    // *UrlRaw = CRUA (/files/...) — é o que volta no PATCH. Sem ela (API antiga)
    // o fallback abaixo, em toPatchPayload, tira a assinatura da versão exibida.
    logoUrlRaw: t.logoUrlRaw || "",
    heroImageUrlRaw: t.heroImageUrlRaw || "",
    footerImageUrlRaw: t.footerImageUrlRaw || "",
...
/**
 * URL de arquivo que vai no PATCH: SEMPRE a crua. Prefere o valor cru guardado
 * no form; se só houver a exibida (assinada), corta o `?token=...&exp=...`.
 * Gravar a assinada fazia a API assinar por cima e a imagem voltar 403.
 * Um campo vazio (imagem removida) vira null.
 */
function fileUrlToSend(raw, signed) {
  const value = String(raw || signed || "").trim();
...
export function toPatchPayload(form = {}, { concluir } = {}) {
  const payload = {
    ...
    logoUrl: fileUrlToSend(form.logoUrlRaw, form.logoUrl),
    heroImageUrl: fileUrlToSend(form.heroImageUrlRaw, form.heroImageUrl),
    footerImageUrl: fileUrlToSend(form.footerImageUrlRaw, form.footerImageUrl),
