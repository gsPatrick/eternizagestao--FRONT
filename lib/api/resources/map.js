...
// ---- sepulturas JÁ DEMARCADAS (camada de referência na demarcação) --------
// Endpoint enxuto do domínio do mapa: devolve só id/código/contorno/âncora e
// APENAS as que têm geometria — a listagem normal de /graves traz proprietário,
// sepultados e contagens, caro demais num cemitério com milhares de covas.
// opts: { bbox:"minLat,minLng,maxLat,maxLng", limit, excludeId, signal, tenant }
export const listReferenceGraves = (cemeteryId, { bbox, limit, excludeId, ...opts } = {}) =>
  api.get("/map/graves", {
    params: { cemeteryId, bbox, limit, excludeId },
    meta: true,
    ...opts,
  });

// item da camada de referência → { id, code, geoPolygon:[[lat,lng],...] }
export function adaptReferenceGrave(g) {
  if (!g) return null;
  const geoPolygon = normalizePolygon(g.geoPolygon);
  if (!geoPolygon) return null;
  return { id: g.id, code: g.code || "—", geoPolygon };
}
