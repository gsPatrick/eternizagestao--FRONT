...
export async function apiFetch(
  path,
  { method = "GET", body, params, tenant, auth = true, meta = false, signal, cache } = {}
) {
  ...
  if (!res.ok || (json && json.success === false)) {
    ...
    // SÓ para chamadas AUTENTICADAS: um 401 de rota pública (auth:false) NÃO deve
    // sequestrar páginas públicas (landing/cidade/portal) para o login.
    if (auth && res.status === 401 && typeof window !== "undefined" && !onLoginRoute()) {
      clearSession();
