...
export async function apiFetch(
  ...
  if (!res.ok || (json && json.success === false)) {
    ...
    // PORTAL_TENANT_MISMATCH é exceção: esse 401 não diz "sua sessão
    // acabou", diz "esta conta não é deste cliente" — ou seja, o header
    // X-Tenant-Subdomain saiu errado. Destruir a sessão por causa disso
    // desloga silenciosamente um usuário perfeitamente válido. O erro é
    // lançado normalmente (a tela trata); só não derruba a sessão.
    // INVALID_PASSWORD idem: é a SENHA ATUAL digitada errada ao trocar de senha,
    // não a sessão. Sem esta exceção, quem erra a senha atual é expulso do
    // painel em vez de ler "senha atual incorreta" na própria tela.
    const naoEhSessaoExpirada =
      err.code === "PORTAL_TENANT_MISMATCH" || err.code === "INVALID_PASSWORD";
    if (
      sentToken &&
      res.status === 401 &&
      !naoEhSessaoExpirada &&
    ...
    // Ação negada pelo PERFIL (RBAC): avisa o app para exibir o modal
    // explicativo, além de lançar o erro para a tela tratar se quiser.
    if (
      (err.code === "INSUFFICIENT_PERMISSION" || err.code === "INSUFFICIENT_ROLE") &&
      typeof window !== "undefined"
    ) {
      window.dispatchEvent(
        new CustomEvent("rbac-denied", { detail: { message: err.message, code: err.code } })
      );
    }
