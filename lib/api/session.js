...
/**
 * Mescla um patch no usuário guardado (ex.: após atualizar o próprio perfil),
 * preservando token/refresh. Retorna o usuário resultante (ou null se não havia
 * usuário nem patch aplicável). Aditivo — não toca em token/refresh.
 */
...
/**
 * Encerra a sessão e vai para a tela indicada com NAVEGAÇÃO DURA.
 *
 * Por que window.location e não o router do Next: o logout com navegação
 * client-side mantém a árvore React viva durante a transição — a casca
 * autenticada (AdminShell/PanelShell) ainda monta por um instante já sem
 * sessão, e qualquer leitura de dado que ela esperava encontrar estoura o
 * "client-side exception". Recarregar de verdade descarta token, estado e
 * componentes de uma vez: é o único jeito de o logout nunca quebrar.
 */
export function logout(destino = "/login") {
  clearSession();
  if (hasWindow()) window.location.assign(destino);
}
